...
");
        ...
        {
            string assembly = null, nuget = null, type = null, cstring = null, script = "create.sql", drop = null;
            ...
            {
                var rv = ResolveNugetPackage(nuget, null);
                rv.Wait();
                assembly = rv.Result;
            }
            ...
            {
                var f = Path.GetFullPath(Path.Combine(".pkg", new AssemblyName(evArgs.Name).Name + ".dll"));
                if(File.Exists(f))
        ...
        private static async Task<string> ResolveNugetPackage(string nuget,string ver)
        {
            Console.Error.WriteLine($"Resolving package {nuget} {ver}");
            var dlNuspecName = $".pkg\\{nuget}.{ver}.nuspec";
            byte[] nuspecData;
            ...
            if (!refs.All(x => File.Exists($".pkg\\{x}")))
            {
                ...
                using (var fs = File.Create($".pkg\\{mydll.Name}"))
            ...
            Task.WaitAll(dd.Select(x => ResolveNugetPackage(x.Id, x.Elem.Attribute(XName.Get("version"))?.Value)).Cast<Task>().ToArray());
            return $".pkg/{refs.First()}";
        }
    }
}
